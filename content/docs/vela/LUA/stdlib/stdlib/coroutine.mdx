...
洛汐文档库的宿主成员索引确认：Vela 表盘宿主提供**完整的 Lua 5.4 协程库**——`coroutine.close`、`create`、`isyieldable`、`resume`、`running`、`status`、`wrap`、`yield` 共 8 个成员均已注册（含 Lua 5.4 的 `close` 与 `isyieldable`），语义遵循 Lua 5.4.0。
